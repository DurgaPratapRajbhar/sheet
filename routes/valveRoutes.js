...
router.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, '../public', 'Valve.html'));
});
...
router.post('/add', async (req, res) => {
  try {
    const checkSheet = new ValveCheckSheet(req.body);
...
router.post('/edit/:id', async (req, res) => {
  try {
    const checkSheet = await ValveCheckSheet.findByIdAndUpdate(
      req.params.id,
      req.body,
...
router.get('/:id', async (req, res) => {
  try {
    const checkSheet = await ValveCheckSheet.findById(req.params.id);
