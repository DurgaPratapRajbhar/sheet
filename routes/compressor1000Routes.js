...
router.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, '../public', 'COMPRESSOR-1000-LPM-CHECK-SHEET.html'));
});
...
router.post('/add', async (req, res) => {
  try {
    const checkSheet = new Compressor1000LpmCheckSheet(req.body);
...
router.post('/edit/:id', async (req, res) => {
  try {
    const checkSheet = await Compressor1000LpmCheckSheet.findByIdAndUpdate(
      req.params.id,
      req.body,
...
router.get('/:id', async (req, res) => {
  try {
    const checkSheet = await Compressor1000LpmCheckSheet.findById(req.params.id);
