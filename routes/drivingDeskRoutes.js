...
router.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, '../public', 'Driving-desk.html'));
});
...
router.post('/add', async (req, res) => {
  try {
    const checkSheet = new DrivingDeskCheckSheet(req.body);
...
router.post('/edit/:id', async (req, res) => {
  try {
    const checkSheet = await DrivingDeskCheckSheet.findByIdAndUpdate(
      req.params.id,
      req.body,
...
router.get('/:id', async (req, res) => {
  try {
    const checkSheet = await DrivingDeskCheckSheet.findById(req.params.id);
