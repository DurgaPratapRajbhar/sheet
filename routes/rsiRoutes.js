...
router.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, '../public', 'RSI.html'));
});
...
router.post('/add', async (req, res) => {
  try {
    const checkSheet = new RsiCheckSheet(req.body);
...
router.post('/edit/:id', async (req, res) => {
  try {
    const checkSheet = await RsiCheckSheet.findByIdAndUpdate(
      req.params.id,
      req.body,
...
router.get('/:id', async (req, res) => {
  try {
    const checkSheet = await RsiCheckSheet.findById(req.params.id);
