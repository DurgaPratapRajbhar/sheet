...
router.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, '../public', 'Panto.html'));
});
...
router.post('/add', async (req, res) => {
  try {
    const checkSheet = new PantoCheckSheet(req.body);
...
router.post('/edit/:id', async (req, res) => {
  try {
    const checkSheet = await PantoCheckSheet.findByIdAndUpdate(
      req.params.id,
      req.body,
...
router.get('/:id', async (req, res) => {
  try {
    const checkSheet = await PantoCheckSheet.findById(req.params.id);
