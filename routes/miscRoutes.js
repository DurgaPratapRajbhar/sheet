...
router.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, '../public', 'Misc-converted-original.html'));
});
...
router.post('/add', async (req, res) => {
  try {
    const checkSheet = new MiscCheckSheet(req.body);
...
router.post('/edit/:id', async (req, res) => {
  try {
    const checkSheet = await MiscCheckSheet.findByIdAndUpdate(
      req.params.id,
      req.body,
...
router.get('/:id', async (req, res) => {
  try {
    const checkSheet = await MiscCheckSheet.findById(req.params.id);
